...
from export_prediction_grouping_input import export_grouping_input


def main() -> None:
    settings = get_settings()
    output_path = Path(settings.alert_module_root) / "data" / "raw" / "master_dataset.csv"
    grouping_script = Path(settings.alert_module_root) / "src" / "monitoring" / "04_prediction_alert_grouping.py"

    print(f"Exporting MongoDB predictions to {output_path}...")
